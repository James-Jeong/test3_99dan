...
void input_width_number( int *width_number){
	...
	while( 1){
		if(( *width_number == DEFAULT_INT) && ( return_value == FAIL)){
			printf("\t| @ 가로 출력 개수 입력 (1~5)\t: ");
			return_value = input_number( width_number);
			if( return_value == FAIL) continue;
		}
		if(( *width_number != DEFAULT_INT) && ( return_value == SUCCESS)) break;
	}
}

/**
 * @fn void display_99dan_result( int width_number)
 ...
 * @return 성공 시 SUCCESS, 실패 시 FAIL 을 반환
 */
static int input_number( int *val){
